...
** 主要内容：易收付平台金融交易处理模块 获取交易参数定义
** 创 建 人：冯炜
** 创建日期：2012-11-08
**
** $Revision: 1.2 $
** $Log: GetTransConf.ec,v $
...
** 修改日志：
****************************************************************/
int GetTransConf(int iTransType, T_EpayConf *ptEpayConf)
{
    EXEC SQL BEGIN DECLARE SECTION;
        int     iHostType;                          /* 交易类型 */
    ...
    {
        WriteLog(ERROR, "交易类型[%d] 查询参数失败!SQLCODE=%d SQLERR=%s",
                 iHostType, SQLCODE, SQLERR);
