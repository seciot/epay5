...
** 修改日志：
****************************************************************/
int RefundPack(MsgRule *ptMsgRule, T_App *ptApp, ISO_data *ptData)
{
    char    szTmpBuf[512+1];
    int     iIndex;

	/* 0域 消息类型 */
    if(SetBit(ptMsgRule, "0220", MSG_ID, 4, ptData) != SUCC)
    {
	...
	/* 3域 交易处理码 */
    if(SetBit(ptMsgRule, "200000", PROC_CODE, 6, ptData) != SUCC)
    {
        WriteLog(ERROR, "交易类型[%d]请求报文第[%d]域组包内容[%s]设置失败!",
                 ptApp->iTransType, PROC_CODE, "200000");
    ...
    /* 61.3域 原交易日期 */
    sprintf(szTmpBuf+iIndex, "%4.4s", ptApp->szHostDate+4);
    iIndex += 4;
    ...
    if(SetBit(ptMsgRule, szTmpBuf, FIELD63, strlen(szTmpBuf), ptData) != SUCC)
    {
        WriteLog(ERROR, "交易类型[%d]请求报文第[%d]域组包内容[%s]设置失败!",
                 ptApp->iTransType, FIELD63, szTmpBuf);
