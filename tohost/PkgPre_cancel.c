...
** 修改日志：
****************************************************************/
int PrecancelPack(MsgRule *ptMsgRule, T_App *ptApp, ISO_data *ptData)
{
    char    szTmpBuf[512+1];
    int     iIndex;

	/* 0域 消息类型 */
    if(SetBit(ptMsgRule, "0100", MSG_ID, 4, ptData) != SUCC)
    {
	...
	/* 25域 服务点条件码 */
    if(SetBit(ptMsgRule, "06", SERVER_CODE, 2, ptData) != SUCC)
    {
        WriteLog(ERROR, "交易类型[%d]请求报文第[%d]域组包内容[%s]设置失败!",
...
** 修改日志：
****************************************************************/
int PrecancelUnpack(MsgRule *ptMsgRule, T_App *ptApp, ISO_data *ptData)
{
	char    szTmpBuf[512+1];
