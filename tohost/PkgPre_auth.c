...
** 修改日志：
****************************************************************/
int PreauthPack(MsgRule *ptMsgRule, T_App *ptApp, ISO_data *ptData)
{
    char    szTmpBuf[512+1];
    int     iIndex;
	...
	/* 3域 交易处理码 */
    if(SetBit(ptMsgRule, "030000", PROC_CODE, 6, ptData) != SUCC)
    {
        WriteLog(ERROR, "交易类型[%d]请求报文第[%d]域组包内容[%s]设置失败!",
...
** 修改日志：
****************************************************************/
int PreauthUnpack(MsgRule *ptMsgRule, T_App *ptApp, ISO_data *ptData)
{
	char    szTmpBuf[512+1];
    ...
    {
        WriteLog(ERROR, "交易类型[%d]应到报文第[%d]域拆解失败iRet:[%d]!",
                ptApp->iTransType, AUTH_ID, iRet);
        strcpy(ptApp->szRetCode, ERR_DATA_FORMAT);
        
        return FAIL;
    }
    sprintf(ptApp->szAuthCode, "%6.6s", szTmpBuf);
