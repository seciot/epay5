...
** 修改日志：
****************************************************************/
int DownByAll(T_App *ptApp, long lTimeOut)
{
    EXEC SQL BEGIN DECLARE SECTION;
    ...
    EXEC SQL
        DECLARE cur_down_all_pos CURSOR FOR
    ...
    if(SQLCODE)
    {
        ...
        WriteLog(ERROR, "声明游标cur_down_all_pos失败!SQLCODE=%d SQLERR=%s", SQLCODE, SQLERR);

        return FAIL;
    }

    EXEC SQL OPEN cur_down_all_pos;
    if(SQLCODE)
    {
        strcpy(ptApp->szRetCode, ERR_SYSTEM_ERROR);

        WriteLog(ERROR, "打开游标cur_down_all_pos失败!SQLCODE=%d SQLERR=%s", SQLCODE, SQLERR);
    ...
    while(1)
    {
        ...
        EXEC SQL
            FETCH cur_down_all_pos INTO :szShopNo, :szPosNo, :szPsamNo, :szIp, :szTelephone, :iTermModule, :iPsamModule;
        if(SQLCODE == SQL_NO_RECORD)
        {
            EXEC SQL CLOSE cur_down_all_pos;
        ...
        else if(SQLCODE)
        {
            ...
            WriteLog(ERROR, "读取游标cur_down_all_pos失败!SQLCODE=%d SQLERR=%s", SQLCODE, SQLERR);
