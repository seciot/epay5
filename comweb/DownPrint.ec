...
int DownPrint(T_App *ptApp)
{
	...
	switch(giDownType)
	{
        ...
        case DOWN_SPECIFY_POS:
            ...
            EXEC SQL
                UPDATE terminal SET down_print = 'Y', print_bitmap = :szBitmap
    ...
    if(SQLCODE)
	{
        ...
        WriteLog(ERROR, "更新终端表 商户[%s] 终端[%s] 应用类型[%d] 机构信息[%s] down_print标志失败!SQLCODE=%d SQLERR=%s",
