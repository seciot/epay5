...
** 主要内容：易收付平台web交易请求接收模块 更新指定应用类型终端
** 创 建 人：冯炜
** 创建日期：2012-12-18
**
** $Revision: 1.4 $
** $Log: DownByAppType.ec,v $
...
** 修改日志：
****************************************************************/
int DownByAppType(T_App *ptApp, long lTimeOut)
{
    EXEC SQL BEGIN DECLARE SECTION;
        int     iAppType;                   /* 应用类型 */
    ...
    EXEC SQL
        DECLARE cur_down_by_apptype CURSOR FOR
    ...
    if(SQLCODE)
    {
        ...
        WriteLog(ERROR, "声明游标cur_down_by_apptype失败!SQLCODE=%d SQLERR=%s", SQLCODE, SQLERR);

        return FAIL;
    }

    EXEC SQL OPEN cur_down_by_apptype;
    if(SQLCODE)
    {
        strcpy(ptApp->szRetCode, ERR_SYSTEM_ERROR);

        WriteLog(ERROR, "打开游标cur_down_by_apptype失败!SQLCODE=%d SQLERR=%s", SQLCODE, SQLERR);
    ...
    while(1)
    {
        ...
        EXEC SQL
            FETCH cur_down_by_apptype INTO :szShopNo, :szPosNo, :szPsamNo, :szIp, :szTelephone, :iTermModule, :iPsamModule;
        if(SQLCODE == SQL_NO_RECORD)
        {
            EXEC SQL CLOSE cur_down_by_apptype;
        ...
        else if(SQLCODE)
        {
            ...
            WriteLog(ERROR, "读取游标cur_down_by_apptype失败!SQLCODE=%d SQLERR=%s", SQLCODE, SQLERR);
