...
** 主要内容：易收付平台web交易请求接收模块 更新功能提示
** 创 建 人：冯炜
** 创建日期：2012-12-18
**
** $Revision: 1.4 $
** $Log: DownFunction.ec,v $
...
int DownFunction(T_App *ptApp)
{
	...
	switch(giDownType)
	{
        ...
        case DOWN_SPECIFY_POS:
            ...
            EXEC SQL
                UPDATE terminal SET down_function = 'Y', function_bitmap = :szBitmap
    ...
    if(SQLCODE)
	{
        ...
        WriteLog(ERROR, "更新终端表 商户[%s] 终端[%s] 应用类型[%d] 机构信息[%s] down_function标志失败!SQLCODE=%d SQLERR=%s",
