...
int DownPayList(T_App *ptApp)
{
	...
	switch(giDownType)
	{
        ...
        case DOWN_SPECIFY_POS:
            ...
            EXEC SQL
                UPDATE terminal SET down_paylist = 'Y'
    ...
    if(SQLCODE)
	{
        ...
        WriteLog(ERROR, "更新终端表 商户[%s] 终端[%s] 应用类型[%d] 机构信息[%s] down_paylist标志失败!SQLCODE=%d SQLERR=%s",
