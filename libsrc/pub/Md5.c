 ...
 *  主要内容：Message-Digest Algorithm 5 信息摘要算法5
 *  创 建 人：
 *  创建日期：
 * ----------------------------------------------------------------
 * $Revision: 1.4 $
 * $Log: Md5.c,v $
 * Revision 1.4  2012/12/14 08:25:13  fengw
 *
 * 1、删除多余空行。
 *
 * Revision 1.3  2012/12/04 06:32:42  chenjr
 * 代码规范化
 *
 * Revision 1.2  2012/11/27 03:08:23  yezt
 * *** empty log message ***
 *
 ...
 * 功    能：MD5接口，生成MD5
 * 输入参数：szSource   信息  
 * 输出参数：szDest     摘要(16个字节,一般需要再转为32个字节的ASC)
 * 返 回 值：
 * 作    者：
 * 日    期：
 * 调用说明：
 ...
 * ----------------------------------------------------------------
 */
void GenMD5(char *szSource, char *szDest)
{
    int i,len;
    MD5Context context;
    unsigned char buff[16];
    ...
    len = strlen(szSource);
    MD5_Init (&context);
    MD5_Update (&context, szSource, len);
    MD5_Final(&context,buff);

    memcpy(szDest, buff, 16);   
}
